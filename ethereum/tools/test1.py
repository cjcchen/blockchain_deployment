...
def SendTxn(acc_idx):
    ...
    #for i in range(5000):
    ip = iplist[acc_idx]
    master = Web3(Web3.HTTPProvider("http://{}:12345".format(ip)))
    #master = GetMiner(iplist[acc_idx])

    txn_send_time = {}
    latency= {}
    assert(len(master.eth.accounts)>0)
    miner_acc = master.eth.accounts[0]
    '''
    for i in range(len(master.eth.accounts)):
        try:
            ...
            print("unlock account fail:{}, e:{}".format(master.eth.accounts[i], e))
    '''
    send_size = 5
    print("start send:",miner_acc)
    def send_some(send_size):
        if count_done:
            return
        try:
            num=send_size
            print("miner:{} send:{}".format(miner_acc, num))
            while True:
                ...
                param={}
                param["from"] = master.eth.accounts[0]
                param["to"] = member.eth.accounts[0]
                param["value"] = 10000
                param["gas"] = 30000
                param["gasPrice"] = 1
                #print("send txn:{}".format(param))
                txn = master.eth.sendTransaction(param)
    ...
    def handle_event(event):
        ...
        try:
            block = master.eth.get_block(event)
        except Exception as e:
            print("get block fail:",e)
            return

        miner = block.miner
        if miner == miner_acc:
            print("miner:{} recv:{} size:{} txn:{} diff:{} height:{}".format( iplist[acc_idx], hash_value, block.size, len(block.transactions), block.difficulty, block.number))
            infx.InsertBlock(iplist[acc_idx], len(iplist), block.number, block.difficulty, block.size)
            ...
            for txn in block.transactions:
                ...
                if  txn in txn_send_time:
                    ...
                    infx.InsertTxn(iplist[acc_idx], len(iplist),latency[txn])
    ...
    block_filter = master.eth.filter('latest')
